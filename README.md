...
## 〇工夫した点
2. パスワードの強度をリアルタイムでチェックし、視覚的フィードバックを通じてユーザーが安全なパスワードを設定しやすいよう工夫しました。
3. トークンを利用することで、高いセキュリティを確保しつつ、ユーザーにとってストレスなくスムーズに利用できるようにしました。
1. 計算処理とデータ保存のAPIを分離し、データの重複確認後に計算APIへリクエストを送ることで、エラーを防ぎつつ安定した処理フローを実現しました。
1. ロード中の読み込みアニメーションを実装し、処理完了前にボタンを押せないよう制御することで、ユーザー操作によるエラーを防ぎました。
2. レスポンシブデザインを徹底し、グリッドシステムを活用した柔軟なレイアウトや、デバイスごとの適切なスタイル調整を行い、スマートフォンでも快適に操作できるよう工夫しました。
3. 時間経過に基づく目標達成率や理想的な進捗を算出し、月末獲得予測や必要獲得数の動的計算を行うことで、精度の高い進捗管理を実現しました。
